    ...
    <group gutter="0" class="gnb_collapse" v-if="!loading">
      <template v-for="(myClass, pindex) in errorClassmate.list">
        ...
        <div class="slide" :class="myClass.checked ? 'animate':''" :key='pindex'>
          <cell-box is-link v-for="(student, index) in myClass.classmates" :key='index' @click.native="$router.push({name: 'errorClassmate_list', params: {name: student.name, studentId: student.id}})">

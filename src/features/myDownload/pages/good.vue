<template>
  <view-box ref="about" body-padding-top="46px">
    ...
    <div>
      <group :gutter='0' title='只展示最近一个月的试卷' v-show='MyDownloadGood.list.length'>
        <cell :title='item.name' v-for='(item, index) in MyDownloadGood.list' :key='index'>
          <div slot="value" style='color:#4cc0be'>
            ...
            <span style='padding:0 5px;line-height:24px;' @click="$router.push({name:'myDownloadGood_detail', params:{id: item.downloadId}})">
      ...
      <div style="text-align:center;padding:10px 0;">
        ...
        <p v-else-if="!MyDownloadGood.list.length" style="font-size:16px;color:#4cc0be">近一月还未有下载记录</p>
...
export default {
  ...
  methods: {
    ...mapActions(['getMyDownloadGood', 'clearMyDownload']),
    _download (item) {
    ...
    _getData () {
      this.clearMyDownload({type: 'good'})
      this.loading = true
      this.getMyDownloadGood({options: {grade: this.grade, subject: this.subject}}).then(() => {
        this.error = false
        this.loading = false
      }).catch((e) => {
        this.error = true
  ...
  beforeRouteEnter (to, from, next) {
    next(vm => {
      if (from.name !== 'myDownloadGood_detail') {
        vm._getData()
      }
    })
  },

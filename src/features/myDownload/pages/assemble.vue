<template>
  <view-box body-padding-top="46px">
    <x-header slot="header" style="width:100%;position:absolute;left:0;top:0;z-index:1;" :left-options="{backText: '组卷列表'}">
    ...
    <div>
      <group :gutter='0' title='只展示最近一个月的试卷' v-show='MyDownloadAssemble.list.length'>
        <cell :title='item.name' v-for='(item, index) in MyDownloadAssemble.list' :key='index'>
          <div style='color:#4cc0be'>
            ...
            <span style='padding:0 5px;line-height:24px;' @click="$router.push({name:'myDownloadAssemble_detail', params:{id: item.downloadId}})">
      ...
      <div style="text-align:center;padding:10px 0;">
        ...
        <p v-else-if="!MyDownloadAssemble.list.length" style="font-size:16px;color:#4cc0be">近一月还未有下载记录</p>
...
export default {
  ...
  methods: {
    ...mapActions(['getMyDownloadAssemble', 'clearMyDownload']),
    _download (item) {
    ...
    _getData () {
      this.clearMyDownload({type: 'assemble'})
      this.loading = true
      this.getMyDownloadAssemble({options: {grade: this.grade, subjectId: this.subjectId}}).then(() => {
        this.error = false
        this.loading = false
      }).catch((e) => {
        this.error = true
